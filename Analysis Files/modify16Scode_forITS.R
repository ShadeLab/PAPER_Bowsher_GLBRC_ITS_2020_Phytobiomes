...
#Change colnames of otu table to match the GLBRC metadata
...
# put taxonomy into its own table
...
#    
#    
#  Fig1A <-ggplot(data = rare, aes(x=raw.read,y=OTU, color=source))+
#    geom_point(size=.1)+
...
#  Fig1A
# 
# ggsave("Figure1A.pdf",Fig1A,width=6,height=4,units=c("in"))
...
#subset for OTUs in the leaf core (492/341 OTUs, with 109 samples = 37169rows)
selected_otus_leaf <- selected_otus[selected_otus$otu %in% fungalcore_sw17,]

#also remove OTUs which were not in phyllosphere
#this would be OTUs that were selected in the above step because core at 1 timepoint,
#but not even present in leaf at other timepoints.
#Then group by samplingdate and taxonomy

#remove rows with occ of zero (for that sampling date, that taxon was absent)
...
leaf_core_abundance <- data.frame(otu = as.factor(row.names(rel_otu_rare)), rel_otu_rare) %>% 
  ...
  geom_line(size=2)+
  geom_point(size=4)+
  scale_color_manual(values = c('red','blue','darksalmon','green4','chocolate4','magenta',
                                'yellow2'))+
...
class(leaf_core_taxonomy_prep$sampling_Rdate)
...
######
#Creating the same figure at the phylum level
...
leaf_core_taxonomy2
#ggsave(file="FigureS2.pdf", leaf_core_taxonomy2, width=4, height=3, units=c("in"))
...
g <- arrangeGrob(leaf_occ_abun_plot,
                 ...
                 leaf_core_taxonomy, 
                 nrow=3) #generates g
#ggsave(file="Figure4.pdf", g, scale=2, width=5, height=10, units=c("in"))
...
#Phyllosphere richness and accumulation through the season
Fig3A <- ggplot(Spec_accum, aes(x=Date, y=Species_Accumulation)) + 
  ...
  guides(shape=FALSE)+
  ggtitle("(A)")+
  theme(plot.title = element_text(margin = margin(b = -17)))
Fig3A
...
#ggsave("Figure3.pdf", f, width=8, height=8, units="in")
...
#test difference between phyllosphere and soil
set.seed(1)
...
### Plot Plant PCoA
Fig2A <- ggplot(plant.points.collapsed, aes(x=Axis1, y=Axis2))+
  geom_point(aes(size=Point_Sizes[2:8]),color='grey0',pch=21,fill="green4") +
  #coord_fixed()+
  coord_cartesian(xlim=c(-.5,0.5),ylim=c(-0.4,0.4))+
  scale_colour_identity() +
  geom_segment(data=plant.points.collapsed, aes(x=Axis1,xend=Axis1+sd_axis1,y=Axis2,yend=Axis2,color="grey0"))+
  geom_segment(data=plant.points.collapsed, aes(x=Axis1,xend=Axis1-sd_axis1,y=Axis2,yend=Axis2,color="grey0"))+
  geom_segment(data=plant.points.collapsed, aes(x=Axis1,xend=Axis1,y=Axis2,yend=Axis2+sd_axis2,color="grey0"))+
  geom_segment(data=plant.points.collapsed, aes(x=Axis1,xend=Axis1,y=Axis2,yend=Axis2-sd_axis2,color="grey0"))+
...
### ggplot Whole PCoA
Fig2B <- ggplot(points.whole, aes(x=Axis1, y=Axis2,color=source,fill=source))+
  #coord_fixed()+
...
Fig2B_leg <- ggplot(points.whole, aes(x=Axis1, y=Axis2,color=source,fill=source))+
  coord_fixed()+
  ...
  guides(fill=guide_legend(override.aes=list(shape=21, color=c(soil="grey0",phyllosphere="grey0"),fill=c(soil="chocolate4",phyllosphere="green4"), size=5)), shape=guide_legend(override.aes = list(size=5)))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))

library(cowplot)
Fig2_Legend <- get_legend(Fig2B_leg)

Fig2 <- grid.arrange(Fig2B,Fig2A,Fig2_Legend,nrow=2)

#ggsave("Figure2.pdf", Fig2, width=9, height=6, units = "in")
...
####################################################
### Look at betadispersion through time ###
####################################################
...
FigS1 <- ggplot(Dispersion.switch.df, aes(x=Date, y=Distance_to_Median,group=Date))+
  geom_boxplot(fill='green4')+
  geom_point()+
...
#ggsave("FigureS1.pdf", FigS3, width = 5, height = 4, units = "in")
...
sort(rownames(rare_core_fungal))
...
#subset fungal table for only samples also in the bactarch data.
...
# Order the samples in the fungal otu table
rare_core_fungal <- rare_core_fungal[,order(colnames(rare_core_fungal))]
# Order the 16s otu table the same way
...
#manually add a tab as the first character of that file. then use in MENA.

#loaded it on 20190905.
#majority was set to 4.
#missing data were kept blank.
#read counts log transformed
#with pearson correlation coeff used as similarity measure.

#The results were then filtered by keeping pairwise correlations with absolute
#LA values greater or equal to 0.93.
...
# The threshold for Pearson is 0.930
# Total Nodes : 239
# Total Links : 359
...
#upload initial network nodes from Cytoscape and add metadata.
...
nodes_withtax$Kingdom.x <- replace_na(nodes_withtax$Kingdom.x, 'Fungi')
#clean up the table
...
test <- merge(rare_core_all, bactarch_tax, by.x='row.names', by.y='otu')


#upload the node attribute file
...
#total number of bacteria-bacteria interactionss
