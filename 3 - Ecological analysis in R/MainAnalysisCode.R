...
#edit column names eotu table to match correspnding sample in the GLBRC metadata
...
#read in taxonomy -----------------------------------------
taxonomy <- read.table('constax_taxonomy/consensus_taxonomy_new.txt', sep='\t',header = T, row.names = 1, fill=TRUE, na.strings=" ")
#fill empty cells with NA. For some reason the mutate_all messes up row names, so grab those first.
OTUnames <- rownames(taxonomy)
taxonomy <- taxonomy %>% mutate_all(na_if,"")
rownames(taxonomy)<-OTUnames




#read in table of confirmed Fungal OTUs. We will subset the OTU table to these otus.
FungalOTUs<-read.csv("switch_fungi.fasta", header=FALSE)
FungalOTUs <- as.data.frame(FungalOTUs[grep(">", FungalOTUs$V1),])
colnames(FungalOTUs)<-c("FungalOTUs")
FungalOTUs = as.data.frame(gsub(pattern = ">", replacement = "", x = FungalOTUs$FungalOTUs))
colnames(FungalOTUs)<-c("FungalOTUs")


#subset the OTU table to include only fungi
otu = otu[row.names(otu) %in% FungalOTUs$FungalOTUs, ]
#removing non-fungi from the taxonomy table
taxonomy = taxonomy[row.names(taxonomy) %in% FungalOTUs$FungalOTUs,]
...
mean(listing$rarefiedreads/min(colSums(otu)))
sd(listing$rarefiedreads/min(colSums(otu)))
...
#subset for OTUs in the leaf core (227 OTUs, with 52 samples = 11804 rows)
...
#left to right: 4,5,3,6,1,7,2




leaf_clusters$stage <- 'A'
leaf_clusters$stage[leaf_clusters$memb_leaf==2] <- 'B'
leaf_clusters$stage[leaf_clusters$memb_leaf==3] <- 'C'
...
leaf_core_taxonomy<-  ggplot(data=leaf_core_taxonomy_prep,aes(x=sampling_Rdate, y=norm_ra, color=Class, group=Class)) +
  ...
  scale_color_manual(values = c('red','dodgerblue3','grey','green4','yellow4','coral3',
                                'greenyellow','indianred4','black','orange'))+
  theme_classic() + theme(strip.background = element_blank(),
...
ggsave(file="test.tiff", g, width=8, height=10, units=c("in"))
...
656/8
#82 OTUs (out of the 227 core OTUs) are unidentified at the genus level.
...
leaf_core_taxonomy3<-  ggplot(data=leaf_core_taxonomy_prep3,aes(x=sampling_Rdate, y=norm_ra, color=Genus, group=Genus)) +
  ...
  geom_errorbar(aes(ymin=norm_ra-n_sd, ymax=norm_ra+n_sd), width=10, size=1.1,
                position=position_dodge(.1))+
  scale_color_manual(values = c('red','blue','grey','green4','chocolate1','magenta',
                                'greenyellow','indianred3','yellow4','violet',
                                'turquoise4','yellow','darksalmon',
                                'orange','purple','orange4','grey80','grey40','grey10'))+
  theme_classic() + theme(strip.background = element_blank(),
                          legend.position = 'bottom',
...
g <- arrangeGrob(leaf_occ_abun_plot,
                 ...
                 leaf_core_taxonomy3, 
                 nrow=3) 
#ggsave(file="Figure4.tiff", g, scale=2, width=6, height=10, units=c("in"))
...
#Do ANOVA 
options(contrasts=c(unordered='contr.sum', ordered='contr.poly'))
Shannon_fit <- aov(Shannon ~ sampling_date, data=map.div.plant)
...
abline(a=0,h=0,b=0)
#test for homogeneity of variance
bartlett.test(Shannon ~ sampling_date, data=map.div.plant)
leveneTest(Shannon ~ sampling_date, data=map.div.plant)
#Test for normality
...
##Pielou
#transform for ANOVA
map.div.plant$Pielou_trans <- sqrt(map.div.plant$Pielou)
...
envfit.phyllo.sub[3,7]<-c("LDMC")
envfit.phyllo.sub[4,7]<-c("NLeaf")
envfit.phyllo.sub[5,7]<-c("C:N")
envfit.phyllo.sub[6,7]<-c("Hgt")
...
Fig2 <- ggplot(plant.points.collapsed, aes(x=Axis1, y=Axis2))+
  ...
  geom_text(data=envfit.phyllo.sub, aes(label=Variable,size=18),nudge_x=.04)+
  labs(tags="B")
...
rare_core_fungal <- as.data.frame(subset(otu_rare, rownames(otu_rare) %in% fungalcore_sw17))
